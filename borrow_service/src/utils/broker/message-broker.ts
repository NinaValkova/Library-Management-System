...
import { MessageType, BorrowEvent, TOPIC_TYPE } from "../../types";

const CLIENT_ID = process.env.CLIENT_ID || "borrow-service";
const GROUP_ID = process.env.GROUP_ID || "borrow-service-group";
...
const connectProducer = async <T>(): Promise<T> => {
  await createTopic(["BorrowEvents", "CatalogEvents"]);
...
const publish = async (data: PublishType): Promise<boolean> => {
  ...
  if (!data.event) {
    throw new Error("Kafka event is required");
  }
    ...
    eachMessage: async ({ topic: receivedTopic, partition, message }) => {
      ...
      if (message.key && message.value) {
        const inputMessage: MessageType = {
          ...
          event: message.key.toString() as BorrowEvent,
